...
//   err = ncmpi_def_dim(ncid, "num_g_blocks", tot_quants[NUM_BLOCKS], &dimids[0]); ERR;
//   err = ncmpi_def_dim(ncid, "XYZ", 3, &dimids[1]); ERR;
//   err = ncmpi_def_dim(ncid, "num_g_particles", tot_quants[NUM_PARTS], &dimids[6]); ERR;
//   err = ncmpi_def_dim(ncid, "num_g_neighbors", tot_quants[NUM_NEIGHBORS], &dimids[7]); ERR;
//   err = ncmpi_def_dim(ncid, "num_g_tets", tot_quants[NUM_TETRAS], &dimids[8]); ERR;
...
//   // quantities
//   err = ncmpi_def_var(ncid, "num_orig_particles", NC_INT, 1, &dimids[0], &varids[4]); ERR;
//   err = ncmpi_def_var(ncid, "num_particles", NC_INT, 1, &dimids[0], &varids[5]); ERR;
//   err = ncmpi_def_var(ncid, "num_tets", NC_INT, 1, &dimids[0], &varids[25]); ERR;

//   // block offsets
//   // encode the offset in the full array of each variable wheree the first
//   // block of each process starts
//   err = ncmpi_def_var(ncid, "block_off_num_particles", NC_INT64, 1, &dimids[0], &varids[9]); ERR;
//   err = ncmpi_def_var(ncid, "block_off_num_tets", NC_INT64, 1, &dimids[0], &varids[26]); ERR;
//   err = ncmpi_def_var(ncid, "block_off_num_rem_gids", NC_INT64, 1, &dimids[0], &varids[29]); ERR;
//   err = ncmpi_def_var(ncid, "block_off_num_neighbors", NC_INT64, 1, &dimids[0], &varids[40]); ERR;
...
//   dimids_2D[1] = dimids[1];
//   err = ncmpi_def_var(ncid, "particles", NC_FLOAT, 2, dimids_2D, &varids[14]); ERR;
//   err = ncmpi_def_var(ncid, "num_neighbors", NC_INT, 1, &dimids[0], &varids[24]); ERR;
//   err = ncmpi_def_var(ncid, "neighbors", NC_INT, 1, &dimids[7], &varids[21]); ERR;
//   err = ncmpi_def_var(ncid, "neigh_procs", NC_INT, 1, &dimids[7], &varids[22]); ERR;
//   err = ncmpi_def_var(ncid, "gids", NC_INT, 1, &dimids[0], &varids[23]); ERR;
//   dimids_2D[0] = dimids[8];
//   dimids_2D[1] = dimids[9];
//   err = ncmpi_def_var(ncid, "tets", NC_INT, 2, dimids_2D, &varids[27]); ERR;
//   if (tot_quants[NUM_REM_GIDS])
//     err = ncmpi_def_var(ncid, "rem_gids", NC_INT, 1, &dimids[10], &varids[30]); ERR;
//   err = ncmpi_def_var(ncid, "vert_to_tet", NC_INT, 1, &dimids[6], &varids[33]); ERR;
...
//     count[0] = 1;
//     err = ncmpi_put_vara_int_all(ncid, varids[4], start, count, &d->num_orig_particles); ERR;
//     err = ncmpi_put_vara_int_all(ncid, varids[5], start, count, &d->num_particles); ERR;
//     err = ncmpi_put_vara_int_all(ncid, varids[25], start, count, &d->num_tets); ERR;
...
//     start[1] = 0;
//     count[1] = 3;
//     err = ncmpi_put_vara_float_all(ncid, varids[11], start, count, d->mins); ERR;
//     err = ncmpi_put_vara_float_all(ncid, varids[12], start, count, d->maxs); ERR;
...
//     count[1] = 3;
//     err = ncmpi_put_vara_float_all(ncid, varids[14], start, count, d->particles); ERR;
...
//     start[0] = block_ofsts[NUM_BLOCKS];
//     count[0] = 1;
//     err = ncmpi_put_vara_int_all(ncid, varids[24], start, count, &num_nbrs[b]); ERR;
//     start[0] = block_ofsts[NUM_NEIGHBORS];
//     count[0] = num_nbrs[b];
//     err = ncmpi_put_vara_int_all(ncid, varids[21], start, count, neighbors); ERR;
//     err = ncmpi_put_vara_int_all(ncid, varids[22], start, count, neigh_procs); ERR;
...
//     start[1] = 0;
//     // casting array of struct of ints to simple 2D array of ints because pnetcdf likes arrays
//     err = ncmpi_put_vara_int_all(ncid, varids[27], start, count, (int *)(d->tets)); ERR;
...
//     start[0] = block_ofsts[NUM_PARTS];
//     count[0] = d->num_particles;
//     err = ncmpi_put_vara_int_all(ncid, varids[33], start, count, d->vert_to_tet); ERR;
...
//   err = ncmpi_inq_var(ncid, varids[11], 0, &type, &ndims, dimids, &natts); ERR;
...
//   // all gids and block index in file of each gid. TODO: not scalable
...
//   err = ncmpi_get_vara_int_all(ncid, varids[23], start, count, all_gids); ERR;
...
//     err = ncmpi_get_vara_int_all(ncid, varids[4], start, count, &(d->num_orig_particles)); ERR;
//     err = ncmpi_inq_varid(ncid, "num_particles", &varids[5]); ERR;
//     err = ncmpi_get_vara_int_all(ncid, varids[5], start, count, &(d->num_particles)); ERR;
//     err = ncmpi_inq_varid(ncid, "num_tets", &varids[25]); ERR;
//     err = ncmpi_get_vara_int_all(ncid, varids[25], start, count, &(d->num_tets)); ERR;
...
//     count[1] = 3;
//     err = ncmpi_inq_varid(ncid, "mins", &varids[11]); ERR;
//     err = ncmpi_get_vara_float_all(ncid, varids[11], start, count, d->mins); ERR;
//     err = ncmpi_inq_varid(ncid, "maxs", &varids[12]); ERR;
//     err = ncmpi_get_vara_float_all(ncid, varids[12], start, count, d->maxs); ERR;
...
//     err = ncmpi_get_vara_float_all(ncid, varids[14], start, count, d->particles); ERR;
...
//     start[0] = start_block_ofst + b;
//     count[0] = 1;
//     err = ncmpi_get_vara_int_all(ncid, varids[24], start, count, &((*num_neighbors)[b])); ERR;
//     if ((*num_neighbors)[b]) {
...
//       err = ncmpi_get_vara_int_all(ncid, varids[21], start, count, (*neighbors)[b]); ERR;
//     }
...
//     err = ncmpi_get_vara_int_all(ncid, varids[27], start, count, (int *)d->tets); ERR;
...
//     count[0] = d->num_particles - d->num_orig_particles;
//     start[0] = (count[0] ? block_ofsts[start_block_ofst + b] : 0);
//     err = ncmpi_inq_varid(ncid, "rem_gids", &varids[30]);
//     // it's possible no remote gids exist, and reading them would be an error
//     // and even if they do exist, make extra sure their dimension is not zero
//     if (err == NC_NOERR)
//     {
//       int dimid;
...
//     err = ncmpi_get_vara_int_all(ncid, varids[33], start, count, d->vert_to_tet); ERR;
//   } // for all blocks
