...
void init_dense(void* b_, const diy::Master::ProxyWithLink& cp, void* args);
void est_dense(void* b_, const diy::Master::ProxyWithLink& cp, void* args);
void recvd_pts(void* b_, const diy::Master::ProxyWithLink& cp, void* args);
void BlockGridParams(dblock_t *dblock, int *block_min_idx, int *block_max_idx, int *block_num_idx,
		     float *grid_phys_mins, float *grid_step_size, float eps, float *data_mins,
                     float *data_maxs, int *glo_num_idx);
void IterateCells(dblock_t *dblock, int *block_min_idx, int *block_num_idx,
                  bool project, float *proj_plane, float *grid_phys_mins, float *grid_step_size,
		  float *data_mins, float *data_maxs, float eps, float mass,
                  const diy::Master::ProxyWithLink& cp);
#ifndef TESS_NO_OPENMP
void IterateCellsOMP(dblock_t *dblock, int *block_min_idx, int *block_num_idx,
                     bool project, float *proj_plane, float *grid_phys_mins, float *grid_step_size,
		     float *data_mins, float *data_maxs, float eps, float mass,
                     const diy::Master::ProxyWithLink& cp);
#endif
void IterateCellsCic(dblock_t *dblock, int *block_min_idx, int *block_num_idx,
                     bool project, float *proj_plane, float *grid_phys_mins, float *grid_step_size,
		     float *data_maxs, float eps, float mass,
                     const diy::Master::ProxyWithLink& cp);
...
int CellGridPts(float *cell_mins, float *cell_maxs, grid_pt_t* &grid_pts, int * &border,
                int& alloc_grid_pts, vector<float> &normals, vector <vector <float> > &face_verts,
                float *data_mins, float *data_maxs, float *grid_phys_mins, float *grid_step_size,
		float mass, float eps, float* site);
int CellInteriorGridPts(int *cell_grid_pts, int *cell_min_grid_idx, float *cell_min_grid_pos,
                        grid_pt_t *grid_pts, int *border, vector<float> &normals,
			vector <vector <float> > &face_verts, float *grid_step_size, float eps,
                        float mass);
bool PtInCell(float *pt, vector<float> &normals,vector <vector <float> > &face_verts, float eps);
...
void DistributeScalarCIC(float *pt, float scalar, vector <int> &grid_idxs,
                         ...
                         float *grid_phys_mins, float eps);
