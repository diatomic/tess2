...
bool my_tet(DBlock& dblock, int t);
bool check_if_delaunay(DBlock& dblock, int t);
void collect_stats(stats_t& stats);
void histogram(struct stats_t& stats);

// debug
float flatness(DBlock& b, int t);
...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < nblocks; i++) {
        if (i == 0) {
            data_min.x = master.block<DBlock>(i)->bounds.min[0];
            data_min.y = master.block<DBlock>(i)->bounds.min[1];
            data_min.z = master.block<DBlock>(i)->bounds.min[2];
            data_max.x = master.block<DBlock>(i)->bounds.max[0];
            data_max.y = master.block<DBlock>(i)->bounds.max[1];
            data_max.z = master.block<DBlock>(i)->bounds.max[2];
        }
        if (master.block<DBlock>(i)->bounds.min[0] < data_min.x)
            data_min.x = master.block<DBlock>(i)->bounds.min[0];
        if (master.block<DBlock>(i)->bounds.min[1] < data_min.y)
            data_min.y = master.block<DBlock>(i)->bounds.min[1];
        if (master.block<DBlock>(i)->bounds.min[2] < data_min.z)
            data_min.z = master.block<DBlock>(i)->bounds.min[2];
        if (master.block<DBlock>(i)->bounds.max[0] > data_max.x)
            data_max.x = master.block<DBlock>(i)->bounds.max[0];
        if (master.block<DBlock>(i)->bounds.max[1] > data_max.y)
            data_max.y = master.block<DBlock>(i)->bounds.max[1];
        if (master.block<DBlock>(i)->bounds.max[2] > data_max.z)
            data_max.z = master.block<DBlock>(i)->bounds.max[2];
    }
...
void display() {
    ...
    if (block_mode) {
        ...
        for (int i = 0; i < nblocks; i++)
            draw_cube(&(master->block<DBlock>(i)->bounds.min[0]),
                      &(master->block<DBlock>(i)->bounds.max[0]),
                      1.0,
                      0.0,
                      1.0);
    }
...
void PrepCellVertRendering() {
    ...
    for (int b = 0; b < nblocks; b++) { // blocks
        ...
        // tets
        for (int t = 0; t < master->block<DBlock>(b)->num_tets; t++) {

            // push voronoi vertex for rendering
            // voronoi vertex is the circumcenter of the tet
            vec3d center;
            circumcenter(&center.x,
                         &(master->block<DBlock>(b)->tets[t]), master->block<DBlock>(b)->particles);

#if 0	// debug purpuses only
            const tet_t& tt = master->block<DBlock>(b)->tets[t];
            float dist = distance(&center.x, &master->block<DBlock>(b)->particles[3*tt.verts[0]]);
            for (int i = 1; i < 4; ++i) {
                float dist2 = distance(&center.x, &master->block<DBlock>(b)->particles[3*tt.verts[i]]);
...
void PrepCellRendering(stats_t& stats) {
    ...
    for (int b = 0; b < nblocks; b++) { // blocks
        ...
        fill_circumcenters(circumcenters, master->block<DBlock>(b)->tets, master->block<DBlock>(b)->num_tets, master->block<DBlock>(b)->particles);

        // for all voronoi cells
        for (int p = 0; p < master->block<DBlock>(b)->num_orig_particles; p++) {

            // tet
            int t = master->block<DBlock>(b)->vert_to_tet[p];

            // skip tets with missing neighbors
            if (t == -1 ||
                master->block<DBlock>(b)->tets[t].tets[0] == -1 ||
                master->block<DBlock>(b)->tets[t].tets[1] == -1 ||
                master->block<DBlock>(b)->tets[t].tets[2] == -1 ||
                master->block<DBlock>(b)->tets[t].tets[3] == -1)
                continue;

            // neighbor edges a vector of (vertex u, tet of vertex u) pairs
            // that neighbor vertex v
            vector< pair<int, int> > nbrs;
            bool finite = neighbor_edges(nbrs, p, master->block<DBlock>(b)->tets, t);
            ...
            for (int i = 0; i < (int)nbrs.size(); ++i) {
                ...
                fill_edge_link(edge_link, p, u, ut, master->block<DBlock>(b)->tets);
                ...
                for (int j = 0; j < (int)edge_link.size(); ++j) {
                    ...
                    circumcenter((float *)&(center.x),
                                 &(master->block<DBlock>(b)->tets[edge_link[j]]), master->block<DBlock>(b)->particles);
            ...
            if (keep) {
                ...
                stats.tot_cells++;
                vols.push_back(volume(p, master->block<DBlock>(b)->vert_to_tet, master->block<DBlock>(b)->tets, master->block<DBlock>(b)->num_tets,
                                      master->block<DBlock>(b)->particles, circumcenters));
...
void PrepTetRendering(stats_t& stats) {
    ...
    for (int b = 0; b < nblocks; b++) { // blocks
        ...
        for (int t = 0; t < master->block<DBlock>(b)->num_tets; t++) {
            ...
            if (!my_tet(*(master->block<DBlock>(b)), t))
                continue;

            for (int v = 0; v < 4; v++) {

                int s = master->block<DBlock>(b)->tets[t].verts[v]; // index pf particle
                vec3d p; // coordinates for tet vertices
                p.x = master->block<DBlock>(b)->particles[3 * s];
                p.y = master->block<DBlock>(b)->particles[3 * s + 1];
                p.z = master->block<DBlock>(b)->particles[3 * s + 2];
...
void PrepSiteRendering(stats_t& stats) {
    ...
    for (int i = 0; i < nblocks; i++) { // blocks
        ...
        // sites
        n = 0;
        for (int j = 0; j < master->block<DBlock>(i)->num_orig_particles; j++) {

            vec3d s;
            s.x = master->block<DBlock>(i)->particles[n];
            s.y = master->block<DBlock>(i)->particles[n + 1];
            s.z = master->block<DBlock>(i)->particles[n + 2];
            n += 3;
            sites.push_back(s);
